...
function AppContent() {
  const [admin, setAdmin] = useState(null); const [active, setActive] = useState('dashboard'); const [stats, setStats] = useState({}); const [orders, setOrders] = useState([]); const [users, setUsers] = useState([]); const [providers, setProviders] = useState([]); const [paymentSettings, setPaymentSettings] = useState({}); const [selectedOrder, setSelectedOrder] = useState(null); const [selectedUser, setSelectedUser] = useState(null); const [orderFilters, setOrderFilters] = useState({ status: '', search: '' }); const [userSearch, setUserSearch] = useState(''); const [loading, setLoading] = useState(true); const [syncing, setSyncing] = useState(false); const [error, setError] = useState(''); const [notice, setNotice] = useState('');
  const loadData = useCallback(async ({ silent = false } = {}) => { if (!silent) setLoading(true); setSyncing(true); setError(''); try { const [statsData, ordersData, usersData, providerData, settingsData] = await Promise.all([api.stats(), api.orders(orderFilters), api.users(userSearch), api.paymentProviders(), api.paymentSettings()]); setStats(statsData); setOrders(ordersData); setUsers(usersData); setProviders(providerData); setPaymentSettings(settingsData); setSelectedOrder((current) => ordersData.find((order) => order.id === current?.id) || ordersData[0] || null); setSelectedUser((current) => usersData.find((user) => user.id === current?.id) || usersData[0] || null); } catch (err) { if (err.status === 401) { localStorage.removeItem('adminToken'); setAdmin(null); return; } setError(err.message || 'Не удалось загрузить админку'); } finally { setLoading(false); setSyncing(false); } }, [orderFilters, userSearch]);
  ...
  useEffect(() => { if (!admin) return undefined; const handle = window.setTimeout(() => loadData({ silent: true }), 350); return () => window.clearTimeout(handle); }, [admin, orderFilters.status, orderFilters.search, userSearch, loadData]);
